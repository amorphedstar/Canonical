...
impl IRTerm {
    pub fn add_local(&self, es: &ES, owned_linked: &mut Vec<S<Linked>>, position: &[Position]) -> (ES, S<Indexed<S<Bind>>>) {
        let bindings = S::new(Indexed {
            params: self.params.iter().enumerate().map(|(i, v)| 
...
fn get_bindings(build: &mut Build, term: &IRSpine, es: ES, position: &[Position]) -> S<Indexed<S<Bind>>> {
    ...
    for (i, arg) in term.args.iter().enumerate() {
        ...
        if es.index_of(&arg.spine.head).is_none() && build.arguments.contains(&arg.spine.head) {
            ...
            let bind = S::new(Bind::new(arg.spine.head.clone(), occurrence));
            // tokens.push(Token { position: occurrence.clone(), declaration: occurrence });
            params.push(bind);
            found = true;
        } else {
            params.push(S::new(Bind::new("*".to_string(), Vec::new())));
        }
    }
