...
/// The `position` of a head-symbol occurrence, together with the `declaration` of its name.
/// A name introduced implicitly on the left-hand side of a rule is declared by its first
/// occurrence, so its `declaration` points at an expression rather than a param or let.
...
pub(crate) fn record_bindings(bindings: &Indexed<S<Bind>>, position: &[Position], binds: &mut BindMap) {
    ...
    for (i, b) in bindings.lets.iter().enumerate() {
        binds.insert(b.downgrade(), extend(position, &[Position::Let(i)]));
    }
}
...
impl IRSpine {
    ...
    pub fn to_body(&self, es: ES, bindings: S<Indexed<S<Bind>>>, owned_linked: Vec<S<Linked>>,
        ...
        let args = self.args.iter().enumerate().map(|(i, t)|
            S::new(t.to_term(&es, &extend(position, &[Position::Arg(i)]), binds, tokens))).collect();

        Meta {
            assignment: Some(Assignment { head, args, bind, changes: Vec::new(), _owned_linked: owned_linked, has_rigid_type: true, var_type: None, compilation_info: CompilationInfo::new() }),
            typ: None,
            gamma: es,
            constraints: Vec::new(),
...
impl IRTerm {
    ...
    pub fn extend_es(&self, es: &ES, owned_linked: &mut Vec<S<Linked>>, params: &[IRVar],
        ...
        record_bindings(bindings.borrow(), position, binds);
        ...
        for (i, d) in self.lets.iter().enumerate() {
            ...
            bindings.borrow_mut().lets[i].borrow_mut().rules = to_rules(&d.rules, &es, owned_linked, &mut owned_bindings,
                &extend(position, &[Position::Let(i)]), binds, tokens);
...
impl IRType {
    ...
    pub fn to_type(&self, es: &ES,
        ...
        let params : Vec<Option<S<TypeBase>>> = self.params.iter().enumerate().map(|(i, t)|
            t.as_ref().map(|t| S::new(t.to_type(&codomain.gamma,
                &extend(position, &[Position::Param(i), Position::Type]), binds, tokens)))).collect();
        let lets : Vec<Option<S<TypeBase>>> = self.lets.iter().enumerate().map(|(i, t)|
             t.as_ref().map(|t| S::new(t.to_type(&codomain.gamma,
                &extend(position, &[Position::Let(i), Position::Type]), binds, tokens)))).collect();
